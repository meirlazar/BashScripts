...
function TOPMENU () {
TYP CLEAR
MOVE 0 10; TYP BLUE; TYP BOLD; echo "L I N U X - B A C K U P  &  R E S T O R E v1.0"
...
lsblk -f | grep -v loop
echo -e "\n\n"
read -p "CHOOSE A PARTITION TO MOUNT(/dev/sdf1)": PARTITION
read -p "CHOOSE A MOUNT POINT (/media/BACKUP)": MPOINT
...
for X in ${BACKUPDIRS}; do
	read -p "BACKUP THE DIRECTORY: $X (Y/N)?": STARTBACKUP
		...
		rsync -az --info=progress2 --delete "${BACKUPSOURCE}/${X}/" "${BACKUPDEST}/${X}/backup.0/"
